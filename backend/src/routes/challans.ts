...
const createChallanSchema = z.object({
  ...
  items: z.array(challanItemInput).min(1, 'At least one product item is required')
});
...
router.put('/:id/status', authenticateToken, requireRole(['Admin', 'Sales', 'Warehouse']), async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { id } = req.params;
